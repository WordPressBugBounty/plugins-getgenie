(() => {
  ...
  var ScriptHandler = class {
    ...
    addGetGenieTriggerBtnForOxygen = (container) => {
      ...
      if (!field.length && jQuery(container).find("#wp-oxygen_vsb_tinymce-wrap").length) {
        ...
        const iframeContainer2 = jQuery(container).find(".mce-container");
        if (iframeContainer2?.length && this.checkVisibility(iframeContainer2[0])) {
          const iframe = iframeContainer2.find("iframe");
          if (iframeContainer2.parent().find(".getGenie-oxygen-iframe").length) {
            iframeContainer2.parent().find(".getGenie-oxygen-iframe").remove();
          }
    ...
    addGetGenieTriggerBtnForFluentCRM = (container, className) => {
      ...
      if (wpEditorContainer) {
        ...
        if (iframeContainer2?.length && this.checkVisibility(iframeContainer2[0])) {
          const iframe = iframeContainer2.find("iframe");
          if (iframeContainer2.parent().find(".getGenie-fluent-iframe").length) {
            iframeContainer2.parent().find(".getGenie-fluent-iframe").remove();
          }
  ...
  // assets/src/admin/js/cptLoadBtn.js
  var iframeContainer;
  ...
  jQuery(document).ready(function($) {
    ...
    if (document.getElementById("post-body")) {
      scriptHandler.cptScriptHandler.init(document.getElementById("post-body"), "getGenie-cpt-textarea");
      scriptHandler.cptScriptHandler.init(document.getElementById("title"), "getGenie-cpt-textarea", false);
    } else {
      scriptHandler.cptScriptHandler.init(document, "getGenie-cpt-textarea");
    }
    scriptHandler.cptScriptHandler.clickHandler();
    const postBody = document.getElementById("wp-content-editor-container");
    if (!postBody) {
      return;
    }
    ...
    const observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        iframeContainer = document.getElementById("content_ifr");
        const iframeWindow = iframeContainer?.contentWindow;
        const iframeDocument = iframeWindow?.document;
        jQuery(iframeDocument).on("click", function(e) {
          e.preventDefault();
          if (wp.data.select("getgenie").contextMenu()?.open) {
            wp.data.dispatch("getgenie").setContextMenu({
              open: false
        ...
        if (!iframeContainer || listenerAdded) {
          return;
        }
        updateCurrentPost();
        wp.data.dispatch("getgenie").setContextMenu({
        ...
        listenerAdded = true;
        iframeContainer.contentDocument.addEventListener("keydown", debounce(function() {
          updateCurrentPost();
        }, 2e3));
      });
    });
    const config = { childList: true, subtree: true };
    observer.observe(postBody, config);
  });
  function updateCurrentPost() {
    wp.data.dispatch("getgenie").setSidebar({
      currentPostContent: iframeContainer?.contentDocument?.body?.innerHTML || ""
    });
  }
})();
