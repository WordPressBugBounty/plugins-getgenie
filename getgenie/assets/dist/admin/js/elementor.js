(() => {
  ...
  // assets/src/admin/js/elementor.js
  var scriptHandler = new ScriptHandler();
  jQuery(document).ready(function($) {
    MutationObserver = window.MutationObserver || window.WebKitMutationObserver;
    var observer = new MutationObserver(function(mutations, observer2) {
      for (let mutation of mutations) {
        scriptHandler.elementorScriptHandler.init(document.getElementById("elementor-panel"), "getGenie-elementor-textarea");
        if (wp.data.select("getgenie").contextMenu()?.open) {
          wp.data.dispatch("getgenie").setContextMenu({
            open: false
          });
    ...
    observer.observe(document.getElementById("elementor-panel"), {
      subtree: true,
      attributeFilter: ["class"],
      attributes: true
    });
    scriptHandler.bricksScriptHandler.clickHandler();
  });
})();
